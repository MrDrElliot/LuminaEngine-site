...
straight [from code](/manual/audio/playback/).
...
The component's fields are settable at runtime, and it exposes a few methods.

<Tabs syncKey="lang">
<TabItem label="C++">

```cpp
SAudioSourceComponent& Source = Context.Get<SAudioSourceComponent>(Entity);
Source.Volume = 0.5f;
...
The methods are identical in both languages.
...
- **[Playing sounds](/manual/audio/playback/)**, firing and steering one-off voices from code.
- **[Mixing & Buses](/manual/audio/mixing/)**, mix groups, an options menu, and reverb.
- **[Audio Settings](/manual/audio/settings/)**, project-wide defaults.
