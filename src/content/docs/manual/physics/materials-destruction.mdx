...
The break parameters below are readable and writable at runtime, so you can tune
them per entity, for example raising the outward speed of the pieces.

<Tabs syncKey="lang">
<TabItem label="C++">

```cpp
SDestructibleComponent& Destruct = Context.Get<SDestructibleComponent>(Entity);
